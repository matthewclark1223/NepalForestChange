...
shapefile_list <- list.files(shapefile_folder, pattern = "\\.shp$", full.names = FALSE)

VDC_List<-gsub(".shp","",shapefile_list,fixed=TRUE)

Nepal$VDC_Jur<-gsub("/","_",Nepal$VDC_Jur)

DoubleCheck<-which(!Nepal$VDC_Jur %in% VDC_List) #was 884
length(DoubleCheck)
...
for(i in DoubleCheck){
  
  print(i)
  print(paste(i/length(DoubleCheck)*100,"%"))
